...
const AnimatedBackground = ({ isDay }) => {
  return (
    // Base layer cross-fades from void black to a moody sunrise gradient
    <div className={`fixed inset-0 transition-colors duration-1000 -z-10 overflow-hidden ${isDay ? 'bg-gradient-to-br from-[#2a1b38] to-[#1a0b16]' : 'bg-[#050505]'}`}>
      
      {/* NIGHT LAYER (Fades out during the Day) */}
      ...
      {/* DAY LAYER (Fades in during the Day) */}
      <div className={`absolute inset-0 transition-opacity duration-1000 ${isDay ? 'opacity-100' : 'opacity-0'}`}>
        <div className="motes-1"></div>
        <div className="motes-2"></div>
        <div className="absolute top-0 left-10 w-64 h-64 bg-orange-500/20 rounded-full blur-3xl"></div>
...
export default function GameBoard() {
  ...
  // This incredibly simple flag drives the entire background engine!
  const isDay = state.phase.startsWith('day_');
  ...
  const renderContent = () => {
    ...
    if (state.phase === 'role_reveal') {
      ...
      return (
        <div className="w-full flex-1 flex flex-col items-center justify-center text-center mt-10">
        ...
        <div className="w-full flex-1 flex flex-col items-center justify-center text-center">
          ...
          <div className="w-full max-w-sm space-y-4">
            {state.dayRecap.map((msg, i) => (
              <div key={i} className="p-6 bg-[#1a1a1a]/80 backdrop-blur-md rounded-xl text-lg font-bold border border-[#222] shadow-xl">
                {msg}
              </div>
            ))}
          </div>
    ...
    if (state.phase === 'day_voting') {
      ...
      return (
        <div className="w-full flex-1 flex flex-col items-center text-center">
          <h2 className="text-slate-400 font-bold uppercase tracking-widest text-[10px] mt-14 mb-2">Town Voting</h2>
          ...
          <div className="w-full max-w-sm mt-8 space-y-2 max-h-[50vh] overflow-y-auto pr-2">
            {alivePlayers.filter(p => p.id !== currentVoter.id).map(p => (
              <button 
                ...
                className="w-full p-4 bg-[#1a1a1a]/80 backdrop-blur-md text-white border border-[#222] rounded-xl font-bold uppercase active:scale-95 transition-transform"
              >
                Vote {p.name}
              </button>
            ))}
            <button 
              ...
              className="w-full p-4 bg-transparent border border-slate-600 text-slate-300 rounded-xl font-bold uppercase mt-4 active:scale-95"
            >
              Pass / No Vote
            </button>
          </div>
          <p className="mt-8 text-slate-500 font-bold text-[10px] uppercase tracking-widest">
  ...
  return (
    <>
      ...
      {/* We make the wrapper transparent so the AnimatedBackground shows through perfectly */}
      <div className="relative min-h-[100dvh] w-full bg-transparent text-white flex flex-col p-6 font-sans">
        {renderBackButton()}
        {renderContent()}
      </div>
    </>
